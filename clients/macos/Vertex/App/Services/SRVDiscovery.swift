...
struct SRVDiscoveryResult: Codable, Sendable {
    ...
    /// (e.g. `sto.exit.vertices.ru. IN TXT "Stockholm, Sweden"`). Key = exit
...
final class SRVDiscovery: Sendable {
    ...
    func resolve(domain: String) async throws -> SRVDiscoveryResult {
        ...
        // (first label) so the UI can render "Stockholm, Sweden" alongside
        // the "sto" code without hardcoding the city table. Each query
