...
final class TunnelManager {
    ...
    func reload() async -> NEVPNStatus {
        ...
        do {
            ...
            log.warning("reload: loadFromPreferences failed: \(error.localizedDescription, privacy: .public)")
        }
    ...
    func loadOrCreate() async throws -> NEVPNStatus {
        ...
        log.info("loadOrCreate: found \(managers.count, privacy: .public) existing VPN configurations")

        let mgr: NETunnelProviderManager
        if let existing = managers.first {
            mgr = existing
            log.info("loadOrCreate: using existing config, enabled=\(mgr.isEnabled, privacy: .public), status=\(String(describing: mgr.connection.status.rawValue), privacy: .public)")
            if let proto = mgr.protocolConfiguration as? NETunnelProviderProtocol {
                log.info("loadOrCreate: providerBundleID=\(proto.providerBundleIdentifier ?? "nil", privacy: .public), server=\(proto.serverAddress ?? "nil", privacy: .public)")
            }
        } else {
    ...
    func configure(config: TunnelConfig, password: String) async throws {
        ...
        log.info("configure: saving preferences (server=\(proto.serverAddress ?? "nil", privacy: .public), brokers=\(config.brokerURLs.count, privacy: .public))...")
        try await mgr.saveToPreferences()
        log.info("configure: save done, reloading...")
        try await mgr.loadFromPreferences()
        log.info("configure: reload done, enabled=\(mgr.isEnabled, privacy: .public), status=\(String(describing: mgr.connection.status.rawValue), privacy: .public)")
    }
    ...
    func start() async throws {
        ...
        log.info("start: calling startVPNTunnel (enabled=\(mgr.isEnabled, privacy: .public), onDemand=\(mgr.isOnDemandEnabled, privacy: .public), status=\(String(describing: mgr.connection.status.rawValue), privacy: .public))...")
        do {
            try mgr.connection.startVPNTunnel()
            log.info("start: startVPNTunnel returned OK")
        } catch {
            log.error("start: startVPNTunnel failed: \(error.localizedDescription, privacy: .public)")
            throw error
        }
    }
    ...
    func stop() async {
        ...
        if mgr.isOnDemandEnabled {
            ...
            do {
                ...
                log.warning("stop: saveToPreferences failed: \(error.localizedDescription, privacy: .public)")
            }
        }
...
enum TunnelManagerError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notConfigured: "VPN is not configured. Set broker URLs and client name first."
        }
    }
}
